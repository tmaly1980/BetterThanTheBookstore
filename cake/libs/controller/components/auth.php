 ...
 */
	function startup(&$controller) {
		...
		error_log("ALLOWED=".print_r($this->allowedActions,true));
		...
		if ($loginAction == $url) {
			if (empty($controller->data) || !isset($controller->data[$this->userModel])) {
				if (!$this->Session->check('Auth.redirect') && env('HTTP_REFERER')) {
					#error_log("SETTING AUTHREDIR=".$controller->referer());
 ...
 */
	function login($data = null) {
		...
		#error_log("LOGGING IN=". print_r($data,true));
 ...
 */
	function redirect($url = null) {
		if (!is_null($url)) {
		#	error_log("SETTING POSTLOGIN REDIR=$url");
			return $this->Session->write('Auth.redirect', $url);
		}
		if ($this->Session->check('Auth.redirect')) {
			$redir = $this->Session->read('Auth.redirect');
			#error_log("AUTH.REDIR=$redir");
		...
		#error_log("URL=$url, REDIR TO $redir");
